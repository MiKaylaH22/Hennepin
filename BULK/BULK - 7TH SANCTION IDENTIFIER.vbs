'STATS GATHERING----------------------------------------------------------------------------------------------------
name_of_script = "BULK - 7TH SANCTION IDENTIFIER.vbs"
start_time = timer
...
ObjExcel.Cells(1, 5).Value = "Total # Sanctions"
ObjExcel.Cells(1, 6).Value = "Date closed for 7th occur."
objExcel.cells(1, 7).Value = "Date closed post 7th occur."

ObjExcel.columns(4).NumberFormat = "00" 'formatting the col to show 2 digit member numbers when the 1st number is 0
ObjExcel.columns(6).NumberFormat = "@" 	'formatting as text
ObjExcel.columns(7).NumberFormat = "@" 	'formatting as text
...
Do
	...
	Else 
        ...
        If total_sanc = "" then total_sanc = 0
        ...
        If (total_sanc < 6 AND seven_occur = "" AND post_seven_occur = "") then 
            SET objRange = objExcel.Cells(excel_row, 1).EntireRow
        ...
        Else 
            ...
            ObjExcel.Cells(excel_row, 5).Value = trim(total_sanc)		
	        ObjExcel.Cells(excel_row, 6).Value = trim(seven_occur)	
	        ObjExcel.Cells(excel_row, 7).Value = trim(post_seven_occur)		
            excel_row = excel_row + 1
