...
name_of_script = "NOTES - APPEAL.vbs"
start_time = timer 
...
'
'DIM Appeal_initial_dialog
'DIM Appeal_summary_completed_dialog
'DIM Appeal_heard_dialog
'DIM Appeal_decision_received_dialog
'DIM maxis_case_number
'DIM appeal_actions
'DIM ButtonPressed
'DIM date_appeal_received
'DIM effective_date
'DIM action_client_is_appealing
'DIM cash_check
'DIM snap_check
'DIM hc_check
'DIM benefits_continuing
'DIM appeal_decision
'DIM proofs_attachments
'DIM other_notes
'DIM actions_needed
'DIM action_taken
'DIM worker_signature
'DIM hearing_date
'DIM appeal_attendence
'DIM hearing_details
'DIM anticipated_date_result
'DIM disposition_of_appeal
'DIM compliance_form_needed
'DIM date_signed_by_judge
'DIM op_needed
'DIM overpayment_amount
'DIM stat_counter
'
BeginDialog Appeal_initial_dialog, 0, 0, 201, 70, "Appeal initial dialog"
...
BeginDialog Appeal_summary_completed_dialog, 0, 0, 351, 195, "Appeal Summary Completed"
...
BeginDialog Appeal_heard_dialog, 0, 0, 346, 140, "Appeal Hearing Info"
...
BeginDialog Appeal_decision_received_dialog, 0, 0, 346, 120, "Appeal decision received"
  EditBox 85, 10, 245, 15, disposition_of_appeal
  EditBox 85, 35, 245, 15, actions_needed
  EditBox 85, 60, 60, 15, date_signed_by_judge
  DropListBox 275, 60, 55, 15, "Select one..."+chr(9)+"Yes"+chr(9)+"NA", compliance_form_needed
...
BeginDialog Appeal_resolution_dialog, 0, 0, 241, 130, "Appeal resolution"
  ...
  Text 10, 10, 115, 10, "Is action needed by HSR/Region?"
...
If appeal_actions = "Appeal Summary Completed" then
	...
	DO		
		Do 
			err_msg = "" 
			Dialog Appeal_summary_completed_dialog
			cancel_confirmation 
...
'Shows dialog and creates and displays an error message if worker completes things incorrectly.  
	Do
		Do 
			err_msg = "" 
			Dialog Appeal_decision_received_dialog
			cancel_confirmation 
	...
	LOOP UNTIL check_for_password(are_we_passworded_out) = False	 
	 ...
	 Call write_bullet_and_variable_in_CASE_NOTE("SNAP compliance form needed", compliance_form_needed)
...
If appeal_actions = "Appeal Resolution" then
	Do
		DO
			err_msg = "" 
			Dialog Appeal_resolution_dialog
			cancel_confirmation 
	...
	Call write_bullet_and_variable_in_CASE_NOTE("Is action needed by HSR/Region?", actions_needed)
