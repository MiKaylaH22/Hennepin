...
EMReadScreen error_msg, 4, 24, 02
IF error_msg = "PROG" THEN	script_end_procedure ("Case must be in PEND II status for script to run, please update MAXIS panels TYPE & PROG (HCRE for HC) and run the script again.")
IF error_msg = "BENE" THEN	script_end_procedure ("Case must be in PEND II status for script to run, please update MAXIS panels TYPE & PROG (HCRE for HC) and run the script again.")
...
BeginDialog appl_detail_dialog, 0, 0, 296, 145, "CASE CORRECTION"
  ...
  DropListBox 80, 25, 65, 15, "Select One:"+chr(9)+"ApplyMN/ECF"+chr(9)+"CAF"+chr(9)+"HCAPP"+chr(9)+"MNSURE/Curam", app_type
  EditBox 230, 25, 60, 15, confirmation_number
  CheckBox 80, 50, 30, 10, "Cash", cash_pend
  CheckBox 115, 50, 30, 10, "CCA", cca_pend
  CheckBox 145, 50, 50, 10, "Emergency", emer_pend
